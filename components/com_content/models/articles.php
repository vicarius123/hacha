...
class ContentModelArticles extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$item_tag = JRequest::getVar('tag');
		// Select the required fields from the table.
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.title,a.*, a.alias, a.introtext, a.fulltext, ' .
		...
		if(!empty($item_tag)){
			$query->join('LEFT', '#__contentitem_tag_map AS tg ON tg.content_item_id = a.id');
			$query->where('tg.tag_id = '.$item_tag);
		}
