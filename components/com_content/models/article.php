...
class ContentModelArticle extends JModelItem
{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				$query = $db->getQuery(true)
					->select(
						$this->getState(
							'item.select', 'a.id, a.asset_id, a.title, a.slider, a.right_ru,a.*, a.alias, a.introtext, a.fulltext, ' .
	...
	function getTags($id, $lang){
		...
		$query = "SELECT t1.* FROM `#__tags` as t1, `#__contentitem_tag_map` as t2 WHERE t1.id = t2.tag_id AND t2.content_item_id = $id";
		
		$db->setQuery($query);
		$results = $db->loadObjectList();
		
		$result = array();
		foreach($results as $key=>$result_a){
			$result[]= '<span><a href="/news?tag='.$result_a->id.'">#'.($lang == 'ru-RU')?$result_a->title:$result_a->title_en.'</a></span>';
		}
		
		return implode('',$result);
	}
	
	function getGallery($id, $lang){
	
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		
		$query = "SELECT image FROM `#__news_newsgalery` WHERE news_id = $id";
	...
	function getSlider($id){
		...
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query = "SELECT data FROM `#__widgetkit` WHERE id = $id";
